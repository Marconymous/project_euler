...
object Problem11 {
  ...
  def findProduct(x: Int, y: Int, dx: Int, dy: Int, n: Int): Int = {
    if (x + dx * n >= 20 || y + dy * n >= 20) 0

    else {
      (0 to n).map(cd => values(x + dx * cd)(y + dy * cd)).product
    }
  }

  def findLargestProduct(n: Int): Int = {
    var largestProduct = 0
    for (i <- 0 until 20) {
      for (j <- 0 until 20) {
        val product = (
          findProduct(i, j, 1, 0, n) max
            findProduct(i, j, 0, 1, n) max
            findProduct(i, j, 1, 1, n) max
            findProduct(i, j, 1, -1, n)
          )
  ...
  def main(args: Array[String]): Unit = {
    val result = findLargestProduct(4)
    println(result)
  }
}
